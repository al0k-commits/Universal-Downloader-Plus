...
[![Build & Release](https://github.com/al0k-commits/Universal-Downloader-Plus/actions/workflows/release.yml/badge.svg)](https://github.com/al0k-commits/Universal-Downloader-Plus/actions/workflows/release.yml)
...
├── .github/workflows/release.yml   # Tag-triggered 3-OS build → signed → GitHub Release
...
|---|---|---|---|
| `BASE_DIR` → `FFMPEG_DIR` | `<repo root>/ffmpeg/` | `<dir of the .exe>/ffmpeg/` | `ffmpeg`, `ffprobe` |
...
```bash
# Windows  → ffmpeg/ffmpeg.exe, ffmpeg/ffprobe.exe   (gyan.dev "essentials" build)
# macOS    → brew install ffmpeg && cp "$(brew --prefix ffmpeg)/bin/"{ffmpeg,ffprobe} ffmpeg/
# Linux    → johnvansickle.com static build → ffmpeg/ffmpeg, ffmpeg/ffprobe
```

CI does this automatically per-OS, so release artifacts always ship with FFmpeg included.
